...
r1 = requests.post("http://localhost:7860/api/reset", json={"task": "easy"})
r2 = requests.post("http://localhost:7860/api/reset", json={"task": "hard"})
d1 = r1.json()
d2 = r2.json()
...
r3 = requests.post("http://localhost:7860/api/auto_step", json={"session_id": sid1})
d3 = r3.json()
print(f"Auto-step session 1: timestep={d3['state']['timestep']}, done={d3['done']}")

# Test auto_step on session 2
r4 = requests.post("http://localhost:7860/api/auto_step", json={"session_id": sid2})
d4 = r4.json()
